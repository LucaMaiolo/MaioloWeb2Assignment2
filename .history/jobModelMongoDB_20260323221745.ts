...
/**
 * Connect to MongoDB and set up the jobs collection.
 * @param dbNameOverride  optional — overrides the default 'jobboard_db' (use for tests)
 * @param reset           if true, drops and recreates the collection (use for tests)
 * @param dbUrl           optional URL override (inject memory server URI in tests)
 */
async function initialize(dbNameOverride?: string, reset?: boolean, dbUrl?: string): Promise<void> {
    try {
      const url = dbUrl ??
      ...
      logger.info('[model] Connected to MongoDB');
      const db = client.db(targetDb);
      const arr = await db.listCollections({ name: 'jobs' }).toArray();
      if (reset && arr.length > 0) {
        await db.collection('jobs').drop();
        logger.info('[model] Dropped jobs collection for reset');
      }
      if (arr.length === 0 || reset) {
        await db.createCollection('jobs', { collation: { locale: 'en', strength: 1 } });
      }
      jobsCollection = db.collection<Job>('jobs');
    } catch (err: unknown) {
      if (err instanceof MongoError)
        throw new DatabaseError('MongoDB connection failed: ' + err.message);
      throw new DatabaseError('Unexpected error during initialize');
    }
  }
...
async function close(): Promise<void> {
  try {
      ...
      if (err instanceof Error)
          ...
          logger.info("Unknown error while closing connection");
  }
}

/**
 * Returns the jobs collection. Used only for unit testing purposes.
 * @throws DatabaseError if the collection has not been initialized
 */
function getCollection(): Collection<Job> {
  if (!jobsCollection)
      throw new DatabaseError("Collection is not defined. DB should be initialized before use.");
...
async function addJob(title:string,description:string,location:string,budget:number): Promise<Job>{
    ...
    try{
        ...
        logger.info('[model] createJob: inserted _id ' + result.insertedId);
        return newJob;    
    }catch (err: unknown) {
        ...
        logger.error('Error inserting job:'); //TODO: more detailed logging
...
async function getAllJobs(statusFilter?:string):Promise<Job[]> {
    ...
    try {
        ...
        logger.info('[model] getAllJobs: ' + results.length + ' results');
        return results;
      } catch (err: unknown) {
...
async function updateJob(title:string, budget: number, status: string):Promise<boolean> {
    if (!title || title.trim().length === 0)
      ...
      try {
        ...
        logger.info('[model] updateJob: updated ' + title);
        return true;
      }
      catch(err: unknown) {
...
async function deleteJob(title:string):Promise<boolean> {
    ...
    try{
        ...
        logger.info('[model] deleteJob: deleted ' + title);
        return true
    }catch (err: unknown) {
        if (err instanceof DatabaseError || err instanceof InvalidInputError) throw err;
...
export {initialize, addJob, getJobByTitle,getAllJobs,deleteJob,updateJob, close, getCollection};
export type { Job }